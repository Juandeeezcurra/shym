...
Estado y plan de trabajo restante. Última actualización: 2026-04-30.
...
`index.html` se actualiza solo en GitHub Pages (refresh y listo).
...
**Backend:**
- `getLastSessionForDay({ day_id })` — para cada `routine_exercise_id` del día, devuelve las series de la última sesión previa (any date < hoy) que use ese mismo `rex_id`. Devuelve `{ rex_id → [sets ordenados por set_number] }`.
...
- Pantalla `screen-train-pick`: botón "Empezar entrenamiento" del Home arranca acá:
  ...
  - Botón "Cambiar rutina" si querés ir de otra
- Pantalla `screen-train`: la sesión en curso
  - Header con día + fecha + botón cambiar
  - Card colapsable de bodyweight (opcional)
  - Por cada ejercicio:
...
- **localStorage autosave:** key `gymtracker:session-draft:<day_id>:<date>`, se escribe debounced en cada input, se limpia al guardar exitosamente. Al entrar al train screen, si hay draft para ese day+date, se restaura silenciosamente con un toast "Continuando sesión".
- **Toggle kg/lb cableado:** los inputs respetan `units.current`. Al enviar al backend, se convierte siempre a kg.
